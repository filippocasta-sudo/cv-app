...
export function CareerGoals({ goals }: { goals: CareerGoalsData }) {
  ...
  const columns = [
    { icon: Compass, title: "Tipo di progetti", items: goals.projectTypes },
    { icon: Building2, title: "Contesto ideale", items: goals.idealContext },
  ];
  ...
  return (
    <motion.section
      ...
      className="print-avoid-break scroll-mt-24 rounded-2xl border border-sage/30 bg-sage-soft/60 p-5 sm:p-7"
    >
      <div className="flex items-center gap-2">
        <Target className="size-4 text-sage" aria-hidden />
      ...
      <div className="mt-6">
        ...
        <ul className="flex flex-wrap gap-2">
          {goals.targetRoles.map((role) => (
            <li
              key={role}
              className="rounded-full border border-sage/40 bg-surface px-3 py-1.5 text-sm font-semibold text-sage-strong"
            >
              {role}
            </li>
          ))}
      ...
      <div className="mt-6 grid gap-6 sm:grid-cols-2">
        {columns.map(({ icon: Icon, title, items }) => (
          <div key={title}>
            <p className="mb-2 inline-flex items-center gap-1.5 text-xs font-semibold tracking-wide text-foreground-muted uppercase">
